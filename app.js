...
async function handleBarcode() {
  ...
  try {
    const productRes = await fetch(`https://raiz-ghvy.onrender.com/api/product/${barcode}`);
    ...
    const analysisRes = await fetch('https://raiz-ghvy.onrender.com/api/analyze-ingredients', {
      method: 'POST',
...
async function handleAnalyze() {
  ...
  try {
    const res = await fetch('https://raiz-ghvy.onrender.com/api/analyze-ingredients', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
async function handleSingleSearch() {
  ...
  try {
    const res = await fetch(`https://raiz-ghvy.onrender.com/api/search?query=${encodeURIComponent(query)}`);
    const data = await res.json();
